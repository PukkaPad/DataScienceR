...
\end{eqnarray*}
$$
Let's practice computing the variance of a dice roll now. 
...
First we need to compute E(X^2). From the definition of expected values, this means we'll take a weighted sum over all possible values of X^2. The weight is the probability of X occurring.
...
Now we need weights. For these we can use any of the three PDF's, (dice_fair, dice_high, and dice_low) we defined in the previous lesson. Using R's ability to multiply vectors componentwise and its function 'sum' we can easily compute E(X^2) for any of these dice. Simply sum the product `dice_sqr * PDF`.  
...
The figure below shows several normal distributions all centered around a common mean 0, but with different standard deviations. As you can see from the color key on the right, the thinner the bell the smaller the standard deviation and the bigger the standard deviation the fatter the bell.
...
1) Which of the following does $Var(X') =$? Here $X'$ represents the sample mean and 'Sum(X_i)' represents the sum of the n samples X_1,...X_n. Assume these samples are independent.

* ` Var(1/n * Sum(X_i))`

2) Which of the following does 'Var(1/n * Sum(X_i)')' equal?

* `1/n^2*Var(Sum(X_i))`

3) Recall that Var is an expected value and expected values are linear. Also recall that our samples X_1, X_2,...,X_n are independent. What does 'Var(Sum(X_i))' equal?

* Variance of the sum = sum of the variance: `Sum(Var(X_i))`

4) Finally, each X_i comes from a population with variance $\sigma^2$. What does 'Sum(Var(X_i))' equal? As before, Sum is taken over n values.
...
5) Standard uniform distributions have variance 1/12. The theory tells us the standard error of means of independent samples of size n would have which standard error?
...
6) Poisson(4) are distributions with variance 4; what standard error would means of random samples of n Poisson(4) have?
...
7) Fair coin flips have variance 0.25; means of random samples of n coin flips have what standard error?

* `1/(2*sqrt(n))`


8) Assume you're taking averages of 10 coin flips and compute the standard error of these means with the theoretical formula you just picked.
...
9) Suppose you had a measurement that was 4 standard deviations from the distribution's mean. What would be the upper bound of the probability of this happening using Chebyshev's inequality?

* `6%`


10) The sample variance estimates what?

* `population variance`

11) The sample variance gets more concentrated around the population variance with larger sample sizes

* `True`

12) The variance of the sample mean is the population variance divided by ?

* `n`

13) The standard error of the sample mean is the sample standard deviation s divided by ?

---
title: "Coursera: Expected Values"
...
```{r lsm, dependson="galton",fig.height=7,fig.width=7, fig.align='center', echo = FALSE, cache = TRUE}
    g <- ggplot(galton, aes(x = child))
    g <- g + geom_histogram(fill = "salmon", 
      binwidth=1, aes(y = ..density..), colour = "black")
    g <- g + geom_density(size = 2)
...
- Is this a valid density? Yes
- Suppose that $X$ follows this density; what is its expected value?  0.5
...
The sample mean is an estimate of the population mean. So the mean of the distribution is $\mu$
...
So odds mean the coin is not a fair coin... $P(heads)=p$. So $P(tail)=1-p$

$Expected-earnings = heads \times p + tail \times (1 - p)$

$Expected-earning = -X \times p + Y \times (1 - p)$

I do not know $p$, but I know the odds of the head. $Odds = p /(1 -p)$
...
$Expected_earnings = -X \times \frac {d}{1+d} + Y \times \frac {1}{1+d})$
